...
func Test_GetBuildLog_NotFound(t *testing.T) {
	server, client := testAPICall(404, "")
	defer server.Close()

	results, err := client.GetBuildLog("123", 456, 789)
...
func Test_GetBuildLog_NotRecognized(t *testing.T) {
	server, client := testAPICall(200, "???")
	defer server.Close()
...
func Test_GetBuildLog_FailedAuthentication(t *testing.T) {
